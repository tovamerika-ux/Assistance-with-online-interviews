@echo off
REM Код конфигурации: (Teams or Phone Link; no Discord; HW: no audio interface, USB-headset)
REM Альтернативный код: (audio_calls_Potato_Samsung_AKG_B1-Krisp_B2-raw_Gain_and_Virtual_Input)

REM Гарнитура Samsung EO-IC100BWEGRU: USBC Headset\Device\Головной телефон\
REM 1. На звук от микрофона своей гарнитуры накладываем шумодав от Krisp, с выхода которого берем очищенный звук и усиливаем его в Voicemeeter. Потом этот звук я буду подавать в ИИ-ассистент
...
REM 3. Усиливаем звук от собеседника (поэтому в имени альтернативного кода конфигурации присутствует "Virtual_Input" по аналогии с названием канала Virtual Inputs\Voicemeeter Input в Voicemeeter, в котором происходит усиление звук от собеседника)
...
REM Выставляем громкость динамиков на максимум:
SoundVolumeView.exe /SetVolume "Voicemeeter Input" 100

REM Выставить громкость динамиков и микрофона проводной гарнитуры при помощи bat не получится, т.к. SoundVolumeView.exe может найти это устройство только по Fiendly ID, которое может меняться при каждом подключении. Поэтому нужно использовать ps1-скрипт
...
start "" "C:\Program Files (x86)\VB\Voicemeeter\voicemeeter8x64.exe" -L"C:\Program Files (x86)\VB\VoicemeeterPotato (Teams or Phone Link; no Discord; HW no audio interface, USB-headset).xml"
timeout /t 1 >nul
