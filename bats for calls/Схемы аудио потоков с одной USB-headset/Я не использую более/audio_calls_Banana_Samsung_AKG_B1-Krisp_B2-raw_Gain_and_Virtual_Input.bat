...
REM Батник написан для Voicemeeter 2.X.X.X, т.е. для Banana. Версия Banana понадобилась из-за того, что мы используем шумодав Krisp, который нужно подать на отдельный виртуальный канал, и усиливаем raw-звук от микрофона (тоже нужен отдельный канал). У VoiceMeeter Standard всего один виртуальный канал, а у Banana - два.
...
taskkill /IM voicemeeterpro_x64.exe /F >nul 2>&1
...
REM Запускаем Voicemeeter Banana с XML-профилем
REM важно: Voicemeeter должен быть полностью закрыт, иначе XML-файл не загрузится
start "" "C:\Program Files (x86)\VB\Voicemeeter\voicemeeterpro_x64.exe" -L"C:\Program Files (x86)\VB\VoicemeeterBanana_calls_mode_Samsung_AKG_headset_B1-Krisp_B2-raw_Gain_and_Virtual_Input.xml"
timeout /t 1 >nul
